...
function ShoppingContent({item}) {
  const [detail, setDetail] = useState([])
  const [implement, setImplement] = useState({total: 0})

  const handleCount = (e) => {
    const {name, value} = e.target
    setImplement((prev) => {return {...prev, [name]: value}})
  }
  const plus = () => {
    ...
    <div className="details col">
      ...
      <div className="row implement">
        <span className="col-auto">
          <button onChange={minus} className="addQuantity">-</button>
          <input name='total' value={implement.total} onChange={handleCount}  className="count" type="number" />
          <button onChange={plus} className="addQuantity">+</button>
