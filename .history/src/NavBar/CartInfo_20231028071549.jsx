...
function CartInfo({ detail }) {
  ...
  const handleDelete = (id) => {
    setDel(prev => [...prev, detail.map(item => (
      detail.id 1== id ? detail.filter(item => (
        item
      )): item
    ))])
  }
            ...
            del?.map((item) => (
              ...
              <>
                <div className="row cartInfo" key={item.id}>
                  ...
                  <div className='col-1'>
                    <button onChange={() => handleDelete(item.id)} className='deleteIcon'><DeleteIcon style={{marginTop: '15px'}}/></button>                    
                  </div>
                </div>
